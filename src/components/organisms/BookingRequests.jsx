...
<Empty
        ...
        description="När företag begär att boka dina fastigheter kommer de att visas här."
      />
    );
  }
...
<span className="font-medium">{booking.totalPrice_c || booking.totalPrice} kr totalt</span>
                    ...
                    {booking.status === "pending" && <div className="flex space-x-3">
                        <button
                            onClick={() => onStatusUpdate(booking.Id, "rejected")}
                            className="px-4 py-2 border border-red-300 text-red-700 rounded-lg hover:bg-red-50 transition-colors"
                            className="px-4 py-2 border border-red-300 text-red-700 rounded-lg hover:bg-red-50 transition-colors">Avböj
                                              <button
                                onClick={() => onStatusUpdate(booking.Id, "confirmed")}
                                className="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">Acceptera
                                                  </button>
                        </button></div>}
                    {booking.status === "confirmed" && <div className="flex items-center space-x-2 text-emerald-600">
                        <ApperIcon name="CheckCircle" className="h-5 w-5" />
                        <span className="font-medium">Bekräftad</span>
                    </div>}
                    {booking.status === "rejected" && <div className="flex items-center space-x-2 text-red-600">
                        <ApperIcon name="XCircle" className="h-5 w-5" />
                        <span className="font-medium">Avböjd</span>
                    </div>}
                </div>
            </motion.div>
        );
